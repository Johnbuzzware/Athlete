...
class AddCardViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        if let amt = ammount{
            ...
            self.btnAddCard.setTitle("Pay Now", for: .normal)
        }
        else{
            self.lblAmount.text = ""
            self.btnAddCard.setTitle("Add Card", for: .normal)
        }
    ...
    @IBAction func payCardBtnPressed(_ sender:Any) {
        if self.tfCardNumber.textColor == UIColor.red || !self.tfCardNumber.isValid() || self.tfExpiryDate.textColor == UIColor.red || !self.tfExpiryDate.isValid() || self.tfcvcNumber.textColor == UIColor.red || !self.tfcvcNumber.isValid(){
            ...
            if cardArray.count > 0{
                let card = self.cardArray.filter { cardModel in
                    if cardModel.last4 == cardParams.last4(){
                        //                    if cardModel.exp_year == cardParams.expYear && cardModel.exp_month == cardParams.expMonth{
                        return true
                        //                    }
                        //                    else{
                        //
                ...
                if card.count > 0{
                    if ammount != nil{
                        ...
                        if self.isSubcribe{
                            ...
                            self.SimplePayment1(pm_id: card.first!.id)
                        }
                    }
                    else{
                        PopupHelper.showAlertControllerWithError(forErrorMessage: "Aleardy Add this card", forViewController: self)
                    }
        ...
        paymentHandler.confirmSetupIntent(paymentIntentParams, with: self) { (status, paymentIntent, error) in
            ...
            case .succeeded:
                ...
                if self.ammount != nil{
                    ...
                    case let controller as WalletViewController:
